...
const BentoGrid = ({
  ...
  return (
    <>
      <ShimmerStyle />
      <div className={cn("grid w-full auto-rows-[22rem] grid-cols-3 gap-4", className)}>{children}</div>
    </>
  )
}
...
const BentoCard = ({
  ...
  >
    {specialOffer && <div className="absolute inset-[2px] bg-white dark:bg-black rounded-xl z-0" />}
      ...
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-6 transition-all duration-300 group-hover:-translate-y-10 relative">
        <Icon
          className={cn(
            ...
            specialOffer && "text-pink-500 dark:text-pink-400",
          )}
        />
